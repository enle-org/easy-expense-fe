...
export default class OrganisationStore {
  ...
  joinOrg = async () => {
    try {
      await axiosInstance.get(
        `${baseUrl}/invite/join?orgId=${this.inviteData.orgID}`,
      );
      localStorage.removeItem('inviteToken');
      runInActionUtil(this, 'success', {
        visible: true,
        message: `You've successfully joined ${this.inviteData.orgName}.`,
      });
  ...
  createOrg = async Component => {
    try {
      if (this.newOrgData.name) {
        ...
        runInActionUtil(this, 'success', {
          visible: true,
          message: `You've successfully created ${this.newOrgData.name}.`,
        });
  ...
  getOrg = async (respData = null) => {
    try {
      const org = {};
      const resp =
        respData || (await axiosInstance.get(`${baseUrl}/organisations`));
  ...
  patchOrg = async () => {
    try {
      ...
      await axiosInstance.patch(`${baseUrl}/organisations/${this.org._id}`, {
        _id: this.org._id,
        name: this.org.name,
        url: baseClientUrl,
        ...patchedOrg,
